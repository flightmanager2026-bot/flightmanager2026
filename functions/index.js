...
const SITE_URL = functions.config().site.url || 'https://flightmanager2026.pl';
...
// POST /createCheckoutSession  { packageId: string }
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: [{
      price_data: {
        currency: 'pln',
        product_data: {
          name: 'Flight Manager 2026 — ' + pkg.name,
          images: [SITE_URL + '/img/logo.boeing.png'],
        },
...
// Webhook Stripe → nalicz środki w Firebase
exports.stripeWebhook = functions.https.onRequest(async (req, res) => {
  ...
  if(event.type === 'checkout.session.completed') {
    ...
    console.log('Payment credited: uid='+uid+' cash='+cashNum+' pts='+ptsNum);
  }
