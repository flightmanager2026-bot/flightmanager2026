...
function showFlightInfo(routeId) {
  ...
  var now = Date.now();
  var isFlying  = ac.status==='flying';
  var isLanded  = ac.status==='landed';
  var isGround  = ac.status==='ground';

  // Progres lotu
  var t = 0;
  if(isFlying && route.startTime && route.duration) {
    t = Math.min(1, (now - route.startTime) / route.duration);
  } else if(isLanded) {
    t = 1;
  }
  ...
  // Dystans
  var dist = route.distKm || Math.round(calcDist(route.fromLat,route.fromLng,route.toLat,route.toLng));

  // Status
  var statusColor = isFlying?'#00d4ff':isLanded?'#00e676':'#5580a0';
  var statusText  = isFlying?'&#9992; W locie':isLanded?'&#10003; Wyladowal':'&#9634; Na ziemi';

  // Pasazerowie
  var cfg = ac.config || {eco:ac.seats||150, biz:0};
  var totalPax = (cfg.eco||0) + (cfg.biz||0) + (cfg.prem||0) + (cfg.first||0);
  ...
  document.getElementById('modal-body').innerHTML =
    ...
    +'<div style="position:absolute;left:0;top:0;height:100%;width:'+pct+'%;background:#00d4ff;border-radius:1px;"></div>'
    ...
    +'<div style="font-size:16px;font-weight:900;color:#a78bfa;">'+eta+'</div>'
    +'<div style="font-size:9px;color:#5580a0;margin-top:2px;letter-spacing:1px;">DO LADOWANIA</div></div>'

    +'<div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:10px;padding:10px;text-align:center;">'
    +'<div style="font-size:16px;font-weight:900;color:#f5a623;">'+(isFlying?speed+' km/h':'0 km/h')+'</div>'
    +'<div style="font-size:9px;color:#5580a0;margin-top:2px;letter-spacing:1px;">PREDKOSC</div></div>'

    +'<div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:10px;padding:10px;text-align:center;">'
    +'<div style="font-size:16px;font-weight:900;color:#00e676;">'+totalPax+'</div>'
    +'<div style="font-size:9px;color:#5580a0;margin-top:2px;letter-spacing:1px;">PASAZEROW</div></div>'

    +'<div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:10px;padding:10px;text-align:center;">'
    +'<div style="font-size:16px;font-weight:900;color:#00d4ff;">'+Math.round((route.duration||0)/60000)+' min</div>'
...
function updateFlightPositions() {
  ...
  G.routes.forEach(function(route) {
    ...
    // Jesli nie ma startTime - samolot stoi, nie animuj
    ...
    if(t>=1) {
      ...
      checkLevelUp(); save();
      showMsg('&#10003; Wyladowal '+ac.model+' ('+route.from+'->'+route.to+')!');
...
function calcETA(route) {
  if(!route.startTime) return 'Na ziemi';
...
/* -- RUNWAY QUEUE -- */
var _runwayQueue=[], _runwayActive=0, _runwayTimer=null;
...
function processRunwayQueue() {
  ...
  _runwayTimer=setInterval(function(){
    ...
    if(_runwayQueue.length===0){clearInterval(_runwayTimer);_runwayTimer=null;}
