...
var STAFF_TYPES = {
  pilot:    { label:'Pilot',        icon:'✈️', color:'#00d4ff', salaryMin:60,  salaryMax:120, desc:'Wymagany do lotu' },
  steward:  { label:'Steward/essa', icon:'👩‍✈️', color:'#a78bfa', salaryMin:30,  salaryMax:70,  desc:'Obsługa pasażerów' },
  mechanic: { label:'Mechanik',     icon:'🔧', color:'#f5a623', salaryMin:40,  salaryMax:90,  desc:'1 na samolot' },
  engineer: { label:'Inżynier',     icon:'👷', color:'#00e676', salaryMin:70,  salaryMax:150, desc:'1 na 3 samoloty' }
};
...
function randomName(type) {
  ...
  return fn+' '+ln+(isFemale&&type==='steward'?'a':'');
}
...
/* Ile slotów na samolot dla danego typu */
function getSlotsPerAc(type, ac) {
  ...
  if(type==='engineer') return 1; // 1 inżynier przypisany do samolotu (ale może być na 3)
  return 1;
}
/* Ile razy 1 inżynier/mechanik może być przypisany */
...
function renderOverview(el) {
  ...
  var html =
    ...
    +'<div style="font-size:10px;color:#5580a0;letter-spacing:1px;margin-top:3px;">PRACOWNIKÓW</div></div>'
  ...
  Object.keys(STAFF_TYPES).forEach(function(type){
    ...
    html +=
      ...
      +(!ok?'<div style="margin-top:8px;font-size:10px;color:#f5a623;background:rgba(245,166,35,0.08);'
        +'border-radius:6px;padding:5px 8px;">⚠ Potrzeba jeszcze '+(needed-count)+' osób</div>':'')
      +'</div>';
  });
  el.innerHTML = html;
}
...
function renderStaffType(el, type) {
  ...
  var html =
    ...
    +(type==='pilot'  ?'<div style="font-size:10px;color:#5580a0;margin-top:2px;">≤6500km → 2 pilotów | ≤12000km → 3 | powyżej → 4</div>':'')
    +(type==='steward'?'<div style="font-size:10px;color:#5580a0;margin-top:2px;">1 FA na każde 50 foteli (EASA CS 26.830)</div>':'')
    ...
    +'font-weight:700;cursor:pointer;font-family:Arial,sans-serif;flex-shrink:0;">🔄 Odśwież</button>'
    +'</div>';

  /* Per-aircraft requirements table */
  if(G.fleet.length>0) {
    html += '<div style="background:rgba(0,0,0,0.25);border-radius:10px;padding:10px;">'
      +'<div style="font-size:9px;color:#5580a0;letter-spacing:2px;margin-bottom:8px;">WYMAGANIA PER SAMOLOT</div>';
    G.fleet.forEach(function(ac){
      var req = getSlotsPerAc(type,ac);
      var has = ((ac.crew||{})[type]||[]).length;
      var acOk = has>=req;
      var info = type==='pilot'?ac.range+' km'
               : type==='steward'?ac.seats+' foteli'
               : type==='engineer'?'max 3 samoloty'
               : '1 samolot';
      html += '<div style="display:flex;align-items:center;justify-content:space-between;'
        +'padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.04);">'
        +'<div style="font-size:12px;color:#e0f0ff;font-weight:600;">'+ac.model+'</div>'
        +'<div style="font-size:10px;color:#5580a0;">'+info+'</div>'
  ...
  if(employed.length>0) {
    ...
    employed.forEach(function(emp, i){
      ...
      var stars=''; for(var s=0;s<5;s++) stars+=s<emp.rating?'<span style="color:#f5a623;">★</span>':'<span style="color:#2a3f5a;">★</span>';

      // Check if fully assigned (pilot/steward/mechanic: assigned to 1, engineer: assigned to 3)
      var maxAssign = getMaxAssignments(type);
      var fullyAssigned = assignedAcs.length >= maxAssign;
      ...
      html +=
        ...
        +'Doświadczenie: <b style="color:#e0f0ff;">'+emp.experience+' lat</b> &nbsp;|&nbsp; '
        +'Pensja: <b style="color:#00e676;">$'+emp.salary+'/24h</b></div>'
        +'</div></div>'

        // Row 2: assignment status badge
        ...
        +'color:'+(isAssigned?'#00e676':'#f5a623')+';display:flex;align-items:center;gap:6px;">'
        +(isAssigned
          ?'✓ Przypisany do: <span style="font-weight:700;">'+assignedAcs.map(function(a){return a.model+' ('+a.reg+')';}).join(', ')+'</span>'
            +(type==='engineer'&&assignedAcs.length<3?' &nbsp;·&nbsp; <span style="color:#5580a0;font-size:10px;">może przyjąć jeszcze '+(3-assignedAcs.length)+' samolot(y)</span>':'')
          :'⚠ Nieprzypisany — kliknij Przypisz')
        +'</div>'
        ...
        +(G.fleet.length>0
          ?'<button onclick="assignStaff(\''+type+'\',\''+emp.id+'\')" '
            ...
            +(fullyAssigned
              ?'background:rgba(0,212,255,0.08);border:1px solid rgba(0,212,255,0.25);color:#00d4ff;">🔄 Zmień przypisanie'
              :isAssigned
                ?'background:rgba(0,212,255,0.08);border:1px solid rgba(0,212,255,0.25);color:#00d4ff;">➕ Dodaj samolot'
  ...
  if(!market.length) {
    html += '<div style="padding:20px;text-align:center;color:#5580a0;font-size:12px;">Brak kandydatów — kliknij 🔄 Odśwież</div>';
  } else {
    market.forEach(function(c,i){
      var stars=''; for(var s=0;s<5;s++) stars+=s<c.rating?'<span style="color:#f5a623;">★</span>':'<span style="color:#2a3f5a;">★</span>';
      html +=
        ...
        +'<div style="font-size:11px;margin-top:3px;">'+stars+' <span style="color:#5580a0;font-size:10px;">'+c.experience+' lat dośw.</span></div>'
...
function hireStaff(type, idx) {
  ...
  G.staff[type].push({id:c.id,name:c.name,experience:c.experience,salary:c.salary,rating:c.rating});
  G.jobMarket[type].splice(idx,1);
  G.jobMarket[type].push(generateCandidate(type));
  save(); showMsg('✓ '+c.name+' zatrudniony/a!');
...
function fireStaff(type, idx) {
  ...
  save(); showMsg(emp.name+' zwolniony/a.');
  var el=document.getElementById('personel-content'); if(el) renderStaffType(el,type);
}
...
function assignStaff(type, empId) {
  ...
  var assignedCount = G.fleet.filter(function(ac){
    return ac.crew&&ac.crew[type]&&ac.crew[type].indexOf(empId)>=0;
  }).length;
  var maxAssign = getMaxAssignments(type);
  ...
  var html =
    ...
    +(type==='engineer'?'Inżynier może być przypisany do max 3 samolotów (aktualnie: '+assignedCount+')'
      :type==='pilot'?'Pilot przypisany do 1 samolotu na raz'
      :type==='steward'?'Stewardesa przypisana do 1 samolotu na raz'
      :'Mechanik przypisany do 1 samolotu na raz')
    +'</div>';
  ...
  G.fleet.forEach(function(ac){
    ...
    html +=
      ...
      +'<div style="font-size:10px;color:#5580a0;margin-top:2px;">'
      +ac.reg+(info?' · '+info:'')+' · obsada: '+crew.length+'/'+slots
      +'</div></div>'
      +(already
        ...
        :full
          ...
          :assignedCount>=maxAssign && type!=='engineer'
            ?'<div style="font-size:10px;color:#5580a0;">Już przypisany</div>'
...
function doAssign(type, empId, acId) {
  ...
  save(); showMsg('✓ Przypisano do '+ac.model+'!');
...
function doUnassign(type, empId, acId) {
  ...
  save(); showMsg('Odpięto od '+ac.model+'.');
  assignStaff(type, empId);
  var el=document.getElementById('personel-content'); if(el) renderStaffType(el,type);
}
...
function canAircraftDepart(ac) {
  ...
  if((crew.pilot||[]).length    <reqP) return {ok:false,reason:'Brak pilotów ('+(crew.pilot||[]).length+'/'+reqP+') → Personel'};
  if((crew.steward||[]).length  <reqS) return {ok:false,reason:'Brak stewardów ('+(crew.steward||[]).length+'/'+reqS+') → Personel'};
  if((crew.mechanic||[]).length <1)    return {ok:false,reason:'Brak mechanika → Personel'};
  if((crew.engineer||[]).length <1)    return {ok:false,reason:'Brak inżyniera → Personel'};
  return {ok:true};
}
...
function paySalaries() {
  ...
  if(total>0){ G.cash-=total; G.lastSalaryPay=now; save(); showMsg('💰 Pensje wypłacone: -$'+total.toLocaleString()); }
}
